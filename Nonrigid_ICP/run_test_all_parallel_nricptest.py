...
for object_name in name_of_objects:
    ...
    for rotate_id in rotate_ids:
        for view_id in first_view_ids:
            while os.path.isfile('./data/'+object_name+'_r'+str(rotate_id)+'_v'+str(view_id)+'_config.yaml')==False:
                pass
            time.sleep(1)
            os.system('python main.py ' + './data/'+object_name+'_r'+str(rotate_id)+'_v'+str(view_id)+'_config.yaml')
